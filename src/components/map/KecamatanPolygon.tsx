...
const KecamatanPolygon: React.FC<KecamatanPolygonProps> = ({
  ...
  // Enhanced coordinate conversion with MULTIPOLYGON support
  const convertCoordinates = () => {
    try {
      const { coordinates, type } = kecamatan.area;

      console.log(`Converting coordinates for ${kecamatan.nama}:`, {
        geometryType: type,
        coordinatesStructure: coordinates,
      });

      // Handle different geometry types
      if (type === "Polygon") {
        // Standard Polygon: coordinates[0] adalah outer ring
        const coords = coordinates[0];
        console.log(`Processing Polygon with ${coords.length} coordinates`);
        ...
        console.log(
          `✅ Successfully converted Polygon with ${converted.length} coordinates for ${kecamatan.nama}`
        );
        return [converted]; // Return array of arrays for consistency
      } else if (type === "MultiPolygon") {
        // MultiPolygon: coordinates adalah array of polygons
        console.log(
          `Processing MultiPolygon with ${coordinates.length} polygons`
        );

        const convertedPolygons = coordinates.map((polygon, polygonIndex) => {
          // Setiap polygon[0] adalah outer ring
          const coords = polygon[0];
          console.log(
            `Processing polygon ${polygonIndex + 1} with ${
              coords.length
            } coordinates`
          );
          ...
          return coords.map((coord, index) => {
            if (!Array.isArray(coord) || coord.length < 2) {
              console.error(
                `Invalid coordinate at polygon ${polygonIndex}, index ${index}:`,
                coord
              );
              return [0, 0] as [number, number];
            }
        ...
        console.log(
          `✅ Successfully converted MultiPolygon with ${convertedPolygons.length} polygons for ${kecamatan.nama}`
        );
      ...
      console.log("Area structure:", kecamatan.area);
      return [];
    }
  };

  const polygonPositions = convertCoordinates();
  ...
  console.log(
    `✅ Rendering ${polygonPositions.length} polygon(s) for ${kecamatan.nama}`
  );
  ...
  // Render multiple polygons for MultiPolygon support
  return (
    <>
      {polygonPositions.map((positions, index) => (
        <Polygon
          key={`${kecamatan.nama}-polygon-${index}`}
          positions={positions}
          pathOptions={{
            ...
            <div className="p-4 min-w-[350px]">
              ...
              <div className="mb-4 pb-3 border-b border-gray-200">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {/* Indicator for MultiPolygon */}
                  {kecamatan.area.type === "MultiPolygon" && (
                    <span className="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded-full">
                      {polygonPositions.length} area
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                {kecamatan.nama_faskes && kecamatan.nama_faskes.length > 0 ? (
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    {kecamatan.nama_faskes.map((namaFaskes, idx) => (
                      <div
                        key={idx}
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-red-600 mt-1">
                        Tingkat malnutrisi dan penyakit menular tinggi di area
                        ini
                      </p>
                    </div>
                  </div>
